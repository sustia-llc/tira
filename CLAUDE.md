...
liveness, not recovery degeneracy), **11**
...
multi-modality `GenerativeModel` directly (koalisi pins git tag `aif-v0.12.0`; the
current release is **`aif-v0.13.0`, cut 2026-08-08** — #53 the deterministic group
read (`group_distribution` / `group_distribution_recording` /
`record_group_action`, plus the defaulted `Aggregator` distribution twins), which
unblocks koalisi #78 (EQ5b) and needs a koalisi re-pin. Its one breaking rider is
...
never matches on the enum. Previous: **`aif-v0.12.0`,
...
- **248 tests** (247 `#[test]` + 1 doctest), edition 2024, 0 clippy warnings under both
