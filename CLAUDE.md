...
**genuinely degenerate**, (η,ω) weakly identifiable and not sampler-limited), **2b**
(β₀/ψ on the positional foraging bandit, #33 — **β₀ partially identifiable
(rank-ordered, prior-shrunk), ψ prior-dominated, NO ridge**, sampler fully converged;
plus the rank-1 law: any rank-1 controlled B is precision-inert, column-varying
deterministic B suffices for a live loop), **3**
(learning-group study), **4** (POMDP sensory/active slots, #40 — **the active slot
dominates the group's blanket identity**: the A1 agreement-seeking announcer moves
recovered α ~10× toward the true member α while sensory distortion is second-order and
the effects don't compose), **5** (certainty-weighted voting), **8** (nested groups,
#41 — **recovery is scale-free**: meta α ≈ inner α ≈ true α at every nesting shape;
CW meta is the one systematic scale effect, ≈ +12% α; fixture contrast = seam
liveness, not recovery degeneracy), **6** (topology-mediated voting, #46 —
**topology moves recovered α only under majority voting, scaling with readout size**
(single-member readout 0.34× all-to-active on the contested fixture); Probabilistic α
is topology-invariant, CW routing is the identity for identical members), **11**
...
group read (`group_distribution` / `group_distribution_recording` /
`record_group_action`, plus the defaulted `Aggregator` distribution twins), which
unblocked koalisi #78 (EQ5b). Its one breaking rider was
compile-level: `AifError` is now `#[non_exhaustive]` and has an `Unsupported`
variant (capability ≠ invalid value) — grep-verified no-op for koalisi, which
never matches on the enum. Earlier: **`aif-v0.12.0`,
...
| `crates/aif/src/group.rs` | `VotingMode` enum (Probabilistic/Deterministic/CertaintyWeighted), `VotingAgent` (discrete votes + confidence-weighted distribution mixing), `GroupAgent` (Markov blanket: sensory→internal→active), `GroupAgentBuilder` (0.8.0: `initial_precision` setter makes `learn_a(true)` groups buildable; CW branch learns via the learning-aware replay path — #4; 0.10.0: `.seed()` reseeds every internal agent — full-pipeline determinism in ALL voting modes, streams voter = `s`, group = `s + 0x9E37_79B9`, internal i = `s + 1 + i`; 0.12.0/#39: `Aggregator` trait (`aggregate`/`aggregate_weighted`/`mode`; `VotingAgent` impl), `GroupAgent<S: Agent = CopyAgent, I: InternalAgent = POMDPAgent, X: Aggregator = VotingAgent>` generic slots — defaults bit-identical, custom slots via `with_slots`/`with_slots_seeded` (group-RNG-only seeding), `internal_agents() -> &[I]`; #41: `InternalAgent for GroupAgent<S, I, VotingAgent>` — nesting: `action_probabilities` = the distribution the voter WOULD have sampled from (CW mixture / normalized counts / uniform-over-winners; extracted private `vote_distribution`/`weighted_mixture`), NO voter draw, sampling moves up; `record_action` no-op; `reseed` = builder scheme (sensory slot not reseeded — documented); `VotingAgent::reseed` pub; aggregate's Probabilistic branch still samples INTEGER counts — f64 weights would change RNG consumption, bit-identity pinned; 0.13.0/#53 **deterministic group read** (flat groups, generic in all three slots): `group_distribution(obs)` — the group's action distribution with NO RNG draw anywhere and NO `last_action` advance (members report `action_probabilities`, votes become the argmax of those, ties to the lowest index; CW goes straight to the mixture) — plus `group_distribution_recording` (same read, each member records its own argmax) and `record_group_action(a)` (validate-then-fan-out commit, so members advance to the GROUP's action; read → decide → commit). Reachability is via two **defaulted** `Aggregator` twins `aggregate_distribution`/`aggregate_weighted_distribution` (`Ok(None)` = no distribution behind the choice ⇒ read reports `AifError::Unsupported`; #51's shared piece), with `VotingAgent::vote_mixture` new and `weighted_mixture` now pub (the inherent names deliberately do NOT shadow the trait's — different return types). Beliefs/Dirichlet counts still advance (it is a no-RNG read, not a side-effect-free one) BUT the pure read does not advance the trial clock: under MeanField a member ignores observations while `last_action` is `None` (`act`'s own t=0 rule), so an uncommitted read loop is a **fixed point** — read → decide → commit is the intended shape, or use the recording read (pinned by `test_uncommitted_reads_stay_at_t0`). RNG-freedom is inherited from the slots, so a NESTED group member does not qualify — its `action_probabilities` runs the sampling member loop) |
...
- **GroupAgent implements Agent** — the same recovery pipeline (record blanket states → `recover_alpha`) applies to single agents and groups. Since #39 the runners are agent-generic (`run_group_simulation`/`run_single_simulation` take `&mut impl Agent`; the group runner delegates to the single one) and `GroupAgent`'s three blanket slots are type parameters with paper-faithful defaults (`S: Agent = CopyAgent`, `I: InternalAgent = POMDPAgent`, `X: Aggregator = VotingAgent`) — the unparameterized type and all existing constructors are bit-identical to before. `InternalAgent` (`action_probabilities`/`record_action`/`reseed`) is dyn-compatible; `Box<dyn InternalAgent>` members are test-pinned bit-identical to concrete ones. `InternalAgent for GroupAgent` (nesting) shipped with ext-8/#41, which is where a group's action-distribution semantics got designed. A **flat** group additionally has an RNG-free read since 0.13.0/#53 — `group_distribution` — whose one open design question, what happens to member `last_action`, is answered by shipping BOTH semantics: the pure read leaves it untouched and pairs with `record_group_action(a)` (members advance to the group's action, caller-resolved), while `group_distribution_recording` advances each member to its own argmax (the deterministic stand-in for `act`'s draw). Ext-4 design note (test-pinned): with a fixed `A` and the MAB's deterministic `B`, a sensory-slot distortion CANNOT move the blanket stream (member beliefs are deltas the observation never reaches) — sensory arms need `learn_a` (or a stochastic `B`) to have any effect.
...
untested). **β₀/ψ are analytically excluded on the MAB**: rank-1 B ⇒ B† uniform ⇒ F_π
policy-constant ⇒ the γ/β loop is provably inert (test-pinned in aif) — measured on
the positional model in **extension 2b** (#33), where the loop is live. All findings
guard-pinned in the binary
...
### 2b. (β₀, ψ) recovery on a live precision loop ✅ STUDY RUN (#33)
Closes the cell extension 2 analytically excluded. Run the study:
`cargo run --release -p reproduce --bin extension2b` (**~29 min** on 12 cores — each
likelihood eval replays 150 trials through an MMP smoother + γ/β loop; report
`docs/extension2b-stochastic-b.md`).

**Phase-0 finding (the rank-1 law, pinned in `tests/ext2b_phase0.rs`)**: the γ/β loop
is inert for ANY rank-1 controlled B (deterministic MAB teleport and uniform action-slip
alike — B† uniform ⇒ F_π policy-constant), while **column-varying DETERMINISTIC
controlled B suffices** for a live loop — no stochastic controlled B needed. The ratified
encoding (owner, 2026-07-30) is the **positional foraging bandit**: walk a line of arms
({left, stay, right}, clamped), good arm on a hazard chain — the strongest F_π signal of
every candidate spiked (~3× sticky slip), zero noise knobs. Dynamics-replay fidelity is
bit-identical (generation ≡ `action_probabilities` replay under MMP+dynamics, incl.
`reset_window` β semantics and learn_a composition).

**Study finding**: **β₀ partially identifiable, ψ prior-dominated, NO ridge.** The
Covariance sampler fully converges in every cell (worst R-hat 1.009 — the only ext-2
joint where the sampler is simply not the story) and corr(β₀,ψ) ≈ 0. Recovered β₀
rank-orders truth (0.5 → 1.03/1.78, 2.0 → 2.23/2.92; prior-shrunk toward 2.70, inflated
by true ψ). Recovered ψ ≈ 3.59–3.69 in every cell = the prior median (3.70) regardless
of true ψ — mechanistically expected: the Table-2 loop runs 16 damped iterations per
timestep, so (1 − 1/ψ)^16 exhausts ψ's transient within each step; ψ is a rate constant
with almost no likelihood trace, while β₀ enters the per-step fixed point. ψ
identification would need transient-rich designs (per-trial `reset_window`, fewer
precision iters, volatility regimes) — future work. All findings guard-pinned
(assert-before-print: convergence, no-ridge, ψ-prior-band, β₀ ordering).

**Implemented in**: `PositionalBanditEnvironment` (lib.rs), `DynamicsParams` +
`ModelParams::with_dynamics` + `build_positional_model` + the `switch_seed` role
(simulation.rs), `tests/ext2b_phase0.rs` (fidelity + encoding pins), and
`crates/reproduce/src/bin/extension2b.rs`.
...
### 4. Sensory and active agents as POMDP agents ✅ STUDY RUN (#40)
The paper uses a CopyAgent (sensory) and VotingAgent (active) as simple rule-based
approximations; §4.1 suggests proper active-inference agents in both slots. Run the
study: `cargo run --release -p reproduce --bin extension4` (< 1 min; report
`docs/extension4-pomdp-blanket.md`).

**Finding**: **the active slot is where the group's blanket identity lives.** The A1
`AgreementAggregator` (two-factor POMDP: controlled announcement × identity-B good-arm
factor; majority-vote + agreement modalities, C prefers agreement) moves recovered group
α ~tenfold (0.040 → 0.385, back toward the true member α = 0.5) and diverges from the
baseline on 62% of steps — a much stronger version of the ext-5 CW direction. Sensory
distortion (`SensoryFilter`, exact-Bayes relay with confusion precision q) is
second-order: q = 0.70 reaches only 16% divergence and nudges α slightly *down*
(monotone in 1−q), and the effects don't compose — (d) tracks (c)'s divergence almost
exactly. A1's agreement channel is *negative* evidence (announce-then-disagree cancels
the vote evidence), which keeps it responsive under churning majorities (sharp-limit
gate 180/180) but lagging after held ones (~run-length lag). With A1 installed the
misspec/aware gap widens (0.240 vs 0.385) — the learning-aware replay becomes
load-bearing for *point* α, stronger than the ext-3 result. All arms run members with
`learn_a` (the #39 inertness constraint); findings guard-pinned (assert-before-print).

**Implemented in**: `crates/reproduce/src/ext4.rs` (`SensoryFilter` S1/S2,
`AgreementAggregator` A1, `build_ext4_group`), `sensory_seed`/`active_seed` roles 5/6
(anti-collision guard extended), and `crates/reproduce/src/bin/extension4.rs`
(master seed `0xE4_2026`, 6 matched-seed cells × 30 reps).
...
### 6. Network communication structures ✅ STUDY RUN (#46, `aif-v0.14.0`)
...
intermediate connections. Run the study:
`cargo run --release -p reproduce --bin extension6` (~36 s; report
`docs/extension6-topology.md`).

**Finding: the topology moves recovered α only under `Deterministic` (majority) voting,
and there it scales with readout size.** On the contested fixture the majority-of-16
all-to-active group recovers α 1.555; a single-member readout (path) recovers 0.530
(0.34×), four hubs (layered) 0.815, sixteen neighbourhood-mixed votes (ring) 1.240 —
strictly ordered by readout, with divergence and arm-0 share following the same order.
Under `Probabilistic` voting recovered α is **topology-invariant** (ratios 0.96–1.04 on
both fixtures) even while the path cell rewrites 60% of the emitted actions: the tally
draws one member's vote (`WeightedIndex(counts)`), routing only reweights that draw, and
identical members cast identically distributed votes. Under `CertaintyWeighted` routing
is the **exact identity** for identical fixed-A members (any row-stochastic mix of
identical distributions is the input; pinned byte-for-byte) — heterogeneous-α, `learn_a`
or per-member-observation rosters are outside that case and not measured. The path cell
is mode-independent across prob/det (one decoded vote ⇒ one non-zero count). This is the
ext-4 answer, not the ext-8 one: the aggregation rule decides whether structure is
visible in α, and topology is visible exactly where the rule is a majority. Findings
guard-pinned (assert-before-print).

**Implemented in**: `crates/aif/src/topology.rs` (`Topology` — member-indexed
row-stochastic adjacency, `from_adjacency(rows, readout, hops)` / `all_to_active(n)` /
`route`; `RoutedAggregator<X: Aggregator = VotingAgent>` — active-slot wrapper, one-hot
vote routing with a no-draw decode for sole-positive rows, RNG-free distribution twins;
default build, no channel — owner decision recorded on #46),
`crates/reproduce/src/ext6.rs`, `crates/reproduce/src/bin/extension6.rs` (master seed
`0xE6_2026`, 4 topologies × 3 modes × 2 fixtures × 30 reps). `communication.rs` stays
the optional carrier for a message-passing **variant** (default-off feature; received
messages have no decision effect); the channel-crate evaluation has no workload until
that variant exists. `petgraph` was not added. The adjacency is member-indexed, so a
roster mutation (ext-9 / #48) must re-key it.
...
### 8. Greater-than-two-scale nesting ✅ STUDY RUN (#41)
Groups of groups: a meta-`GroupAgent` whose internal agents are themselves `GroupAgent`s.
Run the study: `cargo run --release -p reproduce --bin extension8` (~70 s; report
`docs/extension8-nesting.md`).

**Finding**: **the paper's recovery method works recursively — it is scale-free.** Meta-level
α reads like the flat same-headcount α in every probabilistic nesting (4×4/2×8/8×2, ratio
0.95–1.06 on both fixtures) and inner groups recover 0.495–0.508 against true 0.5; nesting
shape is α-invisible even while regrouping moves 31–64% of actions under a contested obs
model. The one systematic scale effect is **CW meta voting** (≈ +12% α, largest divergence)
— the meta-scale analogue of ext-4's active-slot dominance. The fixture contrast is seam
*liveness*, not recovery degeneracy: canonical members nearly always agree (regrouping moves
5–7% of steps) yet recover the TIGHTER α; the concentration is the member's, not the
nesting's (flat 16 already 0.966 arm-0 vs 4×4 meta 0.968). Cross-scale seeding must use
avalanche-mixed substream roles — the builder's `+1+i` offset reused one scale up aliases
streams (negative-pinned). Findings guard-pinned (assert-before-print).

**Implemented in**: `InternalAgent for GroupAgent<S, I, VotingAgent>` (aif 0.12.0 — the
group reports the distribution its voter would have sampled from, no voter draw; sampling
moves up; `VotingAgent::reseed` added), `crates/reproduce/src/ext8.rs`
(`inner_group_seed`/`build_ext8_*`/`run_nested_instrumented` + gates G1–G3), and
