...
| 2 | Recover additional parameters (γ, matrix contents, learning rates) | ✅ **study run** (#29) — `recover_mcmc_vec` (vector MH; scalar #25 is its dim-1 case), `ModelParams`/`log_likelihood_params`, `bin/extension2.rs`, report `docs/extension2-multiparam.md`. Finding: a componentwise-scaled diagonal RW MH **cannot recover** these joints on this fixture — (α,γ) and (α,p) are anti-correlated ridges (pooled r ≈ −0.72/−0.82, a sampler-path magnitude) this sampler does not mix (R-hat ≫ gate, structural to the diagonal proposal), so the marginals are unrecovered; identifiability proper stays **open** (ridge-following sampler = follow-up #30); (η,ω) weakly identifiable. **β₀/ψ correction**: they are NOT well-posed under `PrecisionDynamics` on this MAB — deterministic B ⇒ B† uniform ⇒ F_π policy-constant ⇒ the γ/β loop is provably inert (test-pinned); recovering them needs a **stochastic-B** environment (out of scope) |
