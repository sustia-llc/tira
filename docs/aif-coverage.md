...
| 2 | Recover additional parameters (γ, matrix contents, learning rates) | ❌ — **meaningful since 0.9.0**: γ is dynamic under `PrecisionDynamics` (recovering β₀/ψ becomes a well-posed problem) and learning rates η/ω exist since 0.8.0. **Unblocked by #25**: the MH sampler is in place; remaining work is generalizing `log_likelihood` to a parameter vector (multi-dimensional grid is intractable, MCMC is the path) |
