...
| Policy length 2 | ⚠️ | Engine supports arbitrary `policy_depth` via `with_params`, but all experiment construction sites use `new()` → **depth 1**. Provably equivalent here: deterministic B makes step-2 value independent of step-1 action, so the action marginal is unchanged — and since #8 the claim is **executably pinned** by `test_depth1_depth2_action_marginal_equivalence_mab` (agent.rs; marginals agree < 1e-12 over a 12-step replay). Recovery replays with the same depth-1 model, so generation and inference are consistent. Documented deviation |
...
| Bayesian estimation via MCMC (Turing.jl), posterior **median** | ✅ | Both estimators ship. `recover_alpha` is a **grid MAP** over α ∈ [0.00, 5.00] step 0.01 (fast default, faithful for α < 1). `recover_alpha_mcmc` (extension 1 / #25) is **Metropolis-Hastings** returning the posterior **median** under the same objective (`log_likelihood + half_normal_log_prior`); it reproduces the paper's degenerate-region (α > 1) posterior-median clustering — median ≈ 3.2, between the prior-only 2.7 and the paper's ~4 (Fig 4's high-α band) — which the point-MAP cannot (it saturates at 1.35). Validated by `bin/extension1.rs` / `docs/extension1-mcmc.md` |
...
| Paper element | Status | Where |
|---|---|---|
| Markov blanket group: sensory → internal → active (Fig 3) | ✅ | `group.rs::GroupAgent` — slots generic since #39 with paper-faithful defaults: `CopyAgent` sensory (information transfer), `Vec<POMDPAgent>` internal, `VotingAgent` active |
...
|---|---|---|
| 1 | MCMC parameter estimation (replace grid MAP; posterior median) | ✅ **implemented** (#25) — `recover_alpha_mcmc[_learning]` (Metropolis-Hastings, reflected-at-0 symmetric RW, `McmcConfig`/`McmcResult` with Gelman-Rubin R-hat), same objective as the grid MAP. Reproduces the degenerate-region posterior-median clustering (≈ 3.2, R-hat ≈ 1.00 via a burn-in-adaptive proposal) the point-MAP cannot. `bin/extension1.rs`, report `docs/extension1-mcmc.md` |
| 2 | Recover additional parameters (γ, matrix contents, learning rates) | ✅ **study run** (#29) **+ revisited** (#30 — identifiability settled per joint) — `recover_mcmc_vec` (vector MH; scalar #25 is its dim-1 case) now mode-selected via `ProposalMode`: `JointScale` (the #29 diagonal sampler, default) vs `Covariance` (#30 — Haario-style adaptive covariance in log/logit-transformed space, in-kernel Jacobian); `ModelParams`/`log_likelihood_params`, `bin/extension2.rs` (two matched arms + Q2 4× probe), report `docs/extension2-multiparam.md`. #29 finding stands: the diagonal sampler cannot mix on the (α,γ)/(α,p) anti-correlated ridges (structural, not budget). #30 findings: **(α,γ) partially identifiable — CLOSED** (covariance arm mixes, worst R-hat 16.4 → 1.46; the **product α·γ recovered within 5% in all cells** while factor marginals stay prior-shaped — the data constrain one temperature, not two); **(α,p) not marginally identifiable — CLOSED negative** (4× probe near-converges, R-hat 1.081, onto tight-but-wrong marginals: rec p ≈ 0.36/0.50 vs true 0.8; genuine non-multiplicative degeneracy); **(η,ω) weakly identifiable, not sampler-limited** (covariance does not help; ω → 1 boundary pathology, unfixed by either proposal tested). All guard-pinned in the binary. **β₀/ψ correction**: they are NOT well-posed under `PrecisionDynamics` on this MAB — rank-1 B ⇒ B† uniform ⇒ F_π policy-constant ⇒ the γ/β loop is provably inert (test-pinned); **measured in extension 2b (#33)** on the positional foraging model (live loop): β₀ partially identifiable (rank-ordered, prior-shrunk), ψ prior-dominated (an in-step rate constant — 16 damped Table-2 iterations exhaust its transient per timestep), no β₀–ψ ridge, sampler fully converged — `docs/extension2b-stochastic-b.md` |
...
| 6 | Network communication structures | ✅ study run (#46, 2026-09-16, `aif-v0.14.0`) — `Topology` + `RoutedAggregator` (`topology.rs`, default build, no channel): a member-indexed row-stochastic adjacency routes member votes/distributions before any `Aggregator`; `all_to_active` is byte-identical to the bare aggregator in every mode. **Finding: topology moves recovered α only under `Deterministic` (majority) voting, and there it scales with readout size** — CONTESTED path (readout 1) 0.530 vs all-to-active 1.555 (0.34×), layered (4) 0.815, ring (16) 1.240, strictly ordered. Under `Probabilistic` α is topology-invariant (ratios 0.96–1.04) while the emitted stream moves up to 60% of steps — structural: the tally draws one member's vote and routing only reweights that draw. Under `CertaintyWeighted` routing is the exact identity for identical fixed-A members (any row-stochastic mix of identical distributions is the input); heterogeneous or learning rosters not measured. `communication.rs` stays the optional carrier for a message-passing variant (default-off feature; received messages have no decision effect). Report `docs/extension6-topology.md` |
...
| Learning A (Dirichlet pA, posterior write-back) | ✅ | `update_a` at agent level; group wiring fixed in 0.8.0 (#4: builder precision plumbing + CW branch learns through the learning-aware `action_probabilities`). 0.10.1: read accessors `observation_model()` (learned A) / `pa()` (`Some` iff `learn_a`). 0.11.0: `AgentParams::initial_pa` full-count injection (structured pA at construction, `A ≡ normalize(pA)`; enables structured-A + `learn_a` agents) |
...
| Learning B (pB), D (pD), E (pE) | ✅ | 0.8.0 (#13): Smith Eq. 32–36 family; B/E forms are pymdp/SPM conventions (paper says "analogous", L1035): pB from `s_t ⊗ s_{t−1}` on the taken control, pD once per trial (MeanField: exact o₁-conditioned posterior; MMP: smoothed X₁), pE from q(π). Counts write back into A/B/D/E. MMP+learn_a construction error lifted. 0.10.1: read accessors `transition_model()` (learned B) / `state_prior()` (learned D) + `pb()` / `pd()` counts (`Some` iff the matching `learn_*`). 0.11.0: `AgentParams::initial_pb` full-count injection (`B ≡ normalize(pB)` at construction; mutually exclusive with the `initial_precision_b` scale path) |
...
| Variational free energy F accessor | ✅ | `variational_free_energy()` (0.7.0, #16): MeanField = one-step `−ln p(o_t)` (exact single-factor; mean-field-prior approximation multi-factor); MMP = window F (Eq. 11/19). `policy_free_energies()`: **genuinely per-policy since 0.9.0 under precision dynamics** (per-policy windows spanning observed + future τ; F sums observed τ only per Eq. 19/20, policy-dependent via backward messages from policy-specific futures — varies iff the controlled B is column-varying/non-rank-1; rank-1 controlled B (deterministic MAB, uniform slip) ⇒ B† uniform ⇒ policy-constant, while column-varying deterministic B suffices — the ext-2b rank-1 law, `reproduce/tests/ext2b_phase0.rs`); identical across policies under plain MMP (shared observed-only window, the 0.7.0 behavior, unchanged) |
...
scale) and Appendix Fig A1 (a diagnostic plot). On **Axis 1** (extension coverage) the four
unbuilt extensions are all **filed**: #47 (ext-7) · #48 (ext-9) · #49 (ext-10) · #50 (ext-12);
ext-6 shipped in 0.14.0 (#46). B-novelty (transition-model parameter info gain) shipped in 0.10.0 (#21,
