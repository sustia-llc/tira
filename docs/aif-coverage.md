...
| 4 | Sensory/active agents as proper AIF agents | ❌ study (issue #40) — groundwork shipped in #39: sensory/active slots are generic (`S: Agent`, `X: Aggregator`), injected via `with_slots`/`with_slots_seeded` |
...
| 8 | Greater-than-two-scale nesting (groups of groups) | ❌ study (issue #41) — groundwork shipped in #39: internal storage is generic (`I: InternalAgent = POMDPAgent`), `Box<dyn InternalAgent>` members test-pinned bit-identical; remaining design task is `InternalAgent for GroupAgent` |
